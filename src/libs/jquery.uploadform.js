    ...
    $.fn.uploadform = function(opts)
    {
        var options = $.extend({}, $.fn.uploadform.defaults, opts),
            ...
            addMessageAlert = function() {
                ...
                if(!$('.order__doc').prev().hasClass('message-alert')) {
                    $('.order__doc').before(error_message)
                }
            },

            removeMessageAlert = function(imageField) {
                if($('.order__doc').prev().hasClass('message-alert')){
                    $('.order__doc').prev().remove()
                }
            },
            ...
            addPreview = function(inputField) {
                if(inputField.val()) {
                    ...
                    $('.order__doc').css('background-image', 'url(' + inputField.val().split('?')[1] + ')');
                    $('.order__doc').show();
                } else {
                    //$('.order__doc').attr('style', '')
                }
            };
        ...
        if ($$.length) {
            ...
            addFileLabel = $('<label for="' + options.ajaxUploadFileId + '" class="ajax_upload_file_label photo add_file"><div class="btn btn--micro col-xsm-full"><span class="icon--upload"></span>Прикрепить файл</div></label>');
            addFileInput = $('<input type="file" id="' + options.ajaxUploadFileId + '" class="ajax_upload_file_input">');
            formReset = $('<form />');
            closestForm = $$.closest('form');
            // Otherwise, insert it immediately after the last form:
            imageField.parent().parent().parent().before(addFileLabel);
